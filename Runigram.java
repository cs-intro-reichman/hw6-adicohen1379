...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for(int i=0;i<image.length;i++)
		{
			for(int j=0;j<image[0].length;j++)
			{
				image2[i][image[0].length-j-1]=image[i][j];
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i=0;i<image.length;i++)
		{
			for(int j=0;j<image[0].length;j++)
			{
				image2[image.length-i-1][j]=image[i][j];
	...
	private static Color luminance(Color pixel) {
		...
		Color pixel2=new Color(lum, lum, lum);
		return pixel2;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double newi= (double) image.length/height;
		double newj= (double) image[0].length/width;
		for(int i=0;i<height;i++)
		{
			for(int j=0;j<width;j++)
			{
				image2[i][j]=image[(int) (i*newi)][(int) (j*newj)];
			}
		}
		return image2;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		int rnew=(int)(c1.getRed()*alpha+(c2.getRed()*(1-alpha)));
		int gnew=(int)(c1.getGreen()*alpha+(c2.getGreen()*(1-alpha)));
		int bnew=(int)(c1.getBlue()*alpha+(c2.getBlue()*(1-alpha)));
